...
unsigned uie::menu_hook_impl::execute_by_id_recur(uie::menu_node_ptr parent,unsigned base_id,unsigned max_id,unsigned id_exec)//menu item identifiers are base_id<=N<base_id+max_id (if theres too many items, they will be clipped)
{
	if (parent.is_valid() && parent->get_type()==menu_node_t::type_popup)
	{
		unsigned child_idx,child_num = parent->get_children_count();
...
bool uie::menu_hook_impl::get_description(pfc::string_base & p_out) const {return false;};
...
* - <a href="http://www.foobar2000.org/SDK">foobar2000 1.3 SDK</a>
*
